...
const TripleMetricRowBlock = {
    ...
    fields: [
        ...
        },
        {
            ...
            fields: [
                { name: 'stat', type: 'string', label: 'Stat Number' },
                { name: 'textarea', type: 'markdown', label: 'Description' },
...
const DualStackedTextBlock = {
    name: 'DualStackedTextBlock',
    type: 'object',
    label: 'Stacked Text Columns with Named Button',
    fields: [
        { name: '_section-type', type: 'string', const: 'dual-stacked-text', hidden: true },
        {
            name: 'text',
            type: 'object',
            label: 'Text Areas',
            fields: [
                { name: 'first-textarea_html', type: 'markdown', label: 'First Block Content' },
                { name: 'second-textarea_html', type: 'markdown', label: 'Second Block Content' }
            ]
        },
        {
            name: 'button',
            type: 'object',
            label: 'Button Label Configuration',
            fields: [
                { name: 'button-name', type: 'string', label: 'Display Text on Button' }
            ]
        }
    ]
};

const InfiniteTickerScrollBlock = {
    name: 'InfiniteTickerScrollBlock',
    type: 'object',
    label: 'Horizontal Scrolling Marquee Ticker',
    fields: [
        { name: '_section-type', type: 'string', const: 'infinite-ticker-scroll', hidden: true },
        { name: 'heading', type: 'string', label: 'Marquee Title' },
        { name: 'subheading', type: 'string', label: 'Marquee Subheading Accent' },
        {
            name: 'scroll',
            type: 'list',
            label: 'Scrolling Carousel Items',
            items: {
                type: 'object',
                label: 'Marquee Element',
                fields: [
                    { name: 'text', type: 'string', label: 'Ticker Text Display' },
                    { name: 'image', type: 'image', label: 'Optional Ticker Brand Logo' }
                ]
            }
        }
    ]
...
// ==========================================
// CORE SYSTEM ASSIGNMENT PIPELINE
// ==========================================
    ...
    contentSources: [
        {
            name: 'git',
            provider: 'git',
            options: { repositories: [{ name: 'local', path: '.' }] },
            models: [
                {
                    name: 'Page',
                    type: 'page',
                    ...
                    fields: [
                        ...
                        { name: 'layout', type: 'string', required: true, label: 'Jekyll Layout Template Name' },
                        {
                            name: 'content_blocks',
                            type: 'list',
                            label: 'Visual Page Builder Sections',
                            items: {
                                type: 'model',
                                models: [
                                    'ThreeDeluxeBlock', 'StatsBlock', 'CarouselBlock', 'TextCtaBlock',
                                    'SpacerBlock', 'FormIntakeBlock', 'SimpleHeaderBlock', 'ImageGridTextBlock',
                                    'PrimaryHeroBlock', 'SplitSidebarLayoutBlock', 'ImageGalleryTextListBlock',
                                    'ThreeColumnCtaBlock', 'SimpleTextBlock', 'TextBlockWithButton',
                                    'TripleMetricRowBlock', 'DualStackedTextBlock', 'InfiniteTickerScrollBlock'
                                ]
                            }
                        }
                    ]
